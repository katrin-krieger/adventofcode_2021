...
const dotsAfterFold = (dots, instructions) => {
    ...
    instructions.forEach((instruction) => {
        if (instruction[0] === "y") grid = foldUp(grid, Number(instruction[1]));
...
const foldUp = (grid, line) => {
    ...
    let foldedArray = Array(line)
        .fill()
        .map(() => Array(upper[0].length).fill(0));
...
const foldLeft = (grid, line) => {
    ...
    let foldedArray = Array(left.length + 1)
        .fill()
        .map(() => Array(line).fill(0));
